...
import shap_explainer.parameters as param
from shap_explainer.model import explainer_fn
...
if __name__ == '__main__':
    ...
    parser.add_argument("--domain_id", dest="domain", help="domain id to work on.")
    parser.add_argument("--hit_date", dest="date", help="data date to work on.")
    parser.add_argument("--conf", dest="config", default="/home/roberto/Project/shap/conf/shap_explainer.yaml",
    ...
    logger.info("[RUNNING] SHAP explainer for account %s domain %s" % (account_id, domain_id))
    ...
    if ENV == 'local':
        ...
        packages = {p: 'gs://{bucket}/ml-buy-probability/packages/{package}'.format(bucket=bucket.name,
        ...
        logger.info("[FINISHED] SHAP explainer for account %s domain %s" % (account_id, domain_id))
