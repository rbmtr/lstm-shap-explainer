...
import numpy as np
from ml_buy_probability.train_eval_predict import model_fn
from ml_buy_probability.train_eval_predict import bucketing
from ml_buy_probability.train_eval_predict import padd_and_batch
from ml_buy_probability import parameters
...
class ShapPredictor:
    ...
    def prediction_reshaping(self, data):
        ...
        shift_idx = [self.feature_names['Categorical_features'].index(i)
                     for i in self.feature_names['Categorical_features'] if 'product_lvl_' not in i]

        prod_qty_idx = self.feature_names['Categorical_features'].index('product_qty')
        ...
        sample_data = {'Master_ids': np.transpose([np.arange(n_samples).astype('str')]),
    ...
    def predict_fn(self, sample_data):
        ...
        with tf.name_scope('Dataset_constructor'):
            ...
            def gen_pred():
                for i in range(extended_data['Master_ids'].shape[0]):
                    yield extended_data['Master_ids'][i], \
        ...
        with tf.Session() as sess:
            ...
            # Added master_ids list to be filled later
            ...
            while True:
                ...
                try:
                    ...
                    # Filling master_ids
                    master_ids = np.append(master_ids, [x for x in preds['Master_ids']])
